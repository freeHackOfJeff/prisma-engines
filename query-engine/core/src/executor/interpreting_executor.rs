...
use crate::{CoreResult, Operation, QueryGraphBuilder, QueryInterpreter, QuerySchemaRef, Response, Responses};
...
where
    C: Connector + Send + Sync,
{
    async fn execute(&self, operation: Operation, query_schema: QuerySchemaRef) -> CoreResult<Responses> {
